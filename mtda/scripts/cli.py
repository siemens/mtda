...
class Application:
    ...
    def console_dump(self, args=None):
        ...
        if data is not None:
            if getattr(args, 'condense', False):
                data = self._condense_output(data)
    ...
    def video_cmd(self, args):
        if args.subcommand == "snapshot":
            return self.video_snapshot(args)
        return 1

    def video_snapshot(self, args):
        output = args.output
        if output is None and sys.stdout.isatty():
            print("refusing to write binary data to a terminal; "
                  "use -o FILE or redirect stdout", file=sys.stderr)
            return 1
        data, content_type = self.client().video_snapshot()
        if data is None:
            print("video snapshot not available", file=sys.stderr)
            return 1
        if output is None:
            ...
            with open(output, "wb") as f:
                f.write(data)
            print(f"{output}: {len(data)} bytes written ({content_type})",
                  file=sys.stderr)
        return 0
    ...
    def run(self):
        ...
        output_parser.add_argument(
            "-c", "--condense", action="store_true", default=False,
            help="Condense whitespace and strip VT100 escape sequences",
        )
        s = subsub.add_parser(
            "clear",
        ...
        p = subparsers.add_parser(
            "video",
            help="Interact with the video capture device",
        )
        p.set_defaults(func=cmd)
        subsub = p.add_subparsers(dest="subcommand")
        subsub.required = True
        s = subsub.add_parser(
            "snapshot",
            help="Capture a single frame from the video stream",
        )
        s.add_argument(
            "-o", "--output",
            metavar="FILE",
            type=str,
            default=None,
            help="Path to the output image file (default: stdout)",
        )
