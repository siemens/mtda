...
class WebConsole(RemoteConsole):
    """Async variant of RemoteConsole for use inside the Tornado/asyncio
    event loop.  Uses grpc.aio so the stream can be awaited directly."""
        ...
        while not self.exiting:
            try:
                async with grpc.aio.insecure_channel(target) as channel:
                    ...
                    stream = stub.Subscribe(mtda_pb2.Empty())
                    topics = self._topics()
...
class WebMonitor(WebConsole):
    """Like WebConsole but subscribes to the MON (monitor) topic."""
...
class OpenAPIHandler(RemoteCallHandler):
    async def get(self):
        ...
        try:
            ...
            logger.warning("gRPC timeout fetching agent version for OpenAPI.")
            self.set_status(504)
...
class Service:
    ...
    async def run(self):
        @remote_call
        def mtda_ctrlport(mtda):
            return mtda.ctrlport()
        ...
        # Done after mtda_start so the gRPC server is already listening.
        try:
            ctrlport = await self.blocking_call(mtda_ctrlport)
            remote = await self.blocking_call(mtda_remote)
            output = WebOutput(self.application, self.mtda)
            console = WebConsole(remote, ctrlport, output)
            asyncio.create_task(console.reader())
            monitor = WebMonitor(remote, ctrlport, output)
