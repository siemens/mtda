...
class MultiTenantDeviceAccess:
    ...
    def console_prompt(self, newPrompt=None, **kwargs):
        ...
        session = kwargs.get("session", None)
        self.session_ping(session)
        if self._locked_out(session) is False and \
    ...
    def console_send(self, data, raw=False, **kwargs):
        ...
        if self._locked_out(session) is False and \
           self.console_logger is not None:
            ...
            result = self.console_logger.write(data)
           ...
           self.monitor_logger is not None:
            if not isinstance(data, bytes):
                if raw is False:
                    ...
                    data = data.encode("utf-8")
            result = self.monitor_logger.write(data)
    ...
    def _storage_socket(self, session, size, stream=None):
        if stream is None:
            from mtda.storage.datastream import QueueDataStream
    ...
    def storage_write(self, data, **kwargs):
        ...
        session = kwargs.get("session", None)
        self.session_ping(session)
        result = self._writer.enqueue(data)
    ...
    def _target_on(self, session=None):
        ...
        result = False
        if self._locked_out(session) is False:
    ...
    def target_firmware(self, mode=None, **kwargs):
        ...
        if mode is not None and self._locked_out(session):
    ...
    def notify(self, what, info):
        ...
        result = None
        self.publish("EVT", f"{what} {info}".encode("utf-8"))
    ...
    def start(self):
        ...
        if self.console is not None or self.monitor is not None:
            # Subscribers receive events via gRPC Subscribe stream.
    ...
    def stop(self):
        ...
        # Unblock any active Subscribe streams
        with self._subscriber_lock:
            for q in self._subscribers:
                q.put(None)  # sentinel: tell reader to exit
    ...
    def _locked_out(self, session):
        """True if the device is locked by a session other than the one
        making this call, i.e. the caller should be denied write access
        (console/monitor/keyboard/mouse input, power control, storage
        swap) but may still be allowed read-only access."""
        self.mtda.debug(3, "main._locked_out()")
        ...
        self.mtda.debug(3, f"main._locked_out: {result}")
        return result
